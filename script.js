...
const questionStore = [
  {
    questionId: 1,
    questionText:
      'Ni-Ten-Ichi-Ryu by Photek used which of the following in its introduction?',
    ans1: 'Carnival De Espirit by Bartz',
    ans2: 'Driva Man by Max Roach',
    ans3: 'Rock Dirge by Sly Stone',
    ans4: 'Freedom Song by Halsall',
...

  {
    questionId: 3,
    questionText:
      'Alien Girl by Ed Rush, Optical and Fierce samples which well known sample CD?',
...

  {
    ...
    correctAns: 'Apache by Incredible Bongo',
    img: 'images/4.jpg',
  },

  {
    questionId: 5,
    questionText:
      'What track was used as a second generation sample by Rufuge Cru on 1996&rsquo;s Dark Metal?',
...

  {
    questionId: 8,
    questionText:
      'The vocal sample in Krust&rsquo;s Blaze Dis One is from which artist?',
    ans1: 'Gang Starr',
    ans2: 'LL Cool J',
...

  {
    questionId: 10,
    questionText:
      'Roni Size and DJ Die sampled which of the following on Music Box?',
    ans1: 'Piece of Mind by I. Muhammad',
...
function generateImg() {
  $('.options').css(
    'background-image',
    "url('" + questionStore[progress].img + "')",
  )
}
...
function generatePositiveFeedback() {
  ...
  $('.feedback').show()
  $('.feedback').html(
    `<p>Correct</p><button type=button class="nextButton">Next</button>`,
  )
  next()
}
...
function generateNegativeFeedback() {
  ...
  $('.feedback').show()
  $('.feedback').html(
    `<p>That's Incorrect, the correct answer is ${
      questionStore[progress - 1].correctAns
